...
#   5. Bootstrap Suite: Core PyPI bundle for immediate utility.
#
# FUNCTIONALITY:
...
# Architectural Note: This function serves as the script's input validation layer.
# It detects if the execution is orchestrated by 'rpmbuild' by checking for the
# presence of RPM_BUILD_ROOT, and adjusts behavioral defaults (like skipping
# redundant dependency checks) accordingly.
parse_args() {
  [ $# -eq 0 ] && usage && exit 1
  ...
  # If RPM_BUILD_ROOT is set, we assume we are part of a governed build process.
...
# Architectural Note: Centralizing path derivation ensures that all functions
# (Prep, Compile, Install) share a consistent view of the target hierarchy.
setup_globals() {
  # Version Derivations: Extract major and minor components for feature gating.
  ...
  # This is used for generating binary names and linking shared libraries.
  ...
  if [ -n "${CONF[DISTRIB_OVERRIDE]:-}" ]; then
    ...
    local os_v
    os_v=$(grep -oP '(?<=VERSION_ID=")\d+' /etc/os-release || echo "unknown")
  ...
  # Path Registry: Mapping the build-time staging to production-time prefixes.
  ...
  # This ensures that 'make install' populates the correct RPM staging area.
  ...
  # ARCHIVE_PATH: The final redistributable artifact path.
  ...
  # Architectural Note: This prevents file owner/group pollution in production binaries.
...
# Supports both DNF (RedHat) and APT (Ubuntu/Debian) for cross-platform portability.
# Architectural Note: This phase ensures the 'Host Build Readiness'. It
# dynamically translates package names between distributions to maintain a
# unified build logic across different Linux ecosystems.
check_dependencies() {
  ...
  if command -v dnf &> /dev/null; then
    pkg_mgr="dnf"
  elif command -v apt-get &> /dev/null; then pkg_mgr="apt"; fi

  log "Checking host dependencies (${pkg_mgr:-unknown})..."

  # Base package list (RedHat naming)
  local rpms=(
    autoconf automake bzip2-devel gcc gdbm-devel git-core
    glibc-devel libffi-devel libuuid-devel make ncurses-devel
    pkgconf-pkg-config readline-devel xz-devel wget
  )
  ...
  for pkg in "${rpms[@]}"; do
    ...
    if [ "$pkg_mgr" == "apt" ]; then
      case "$pkg" in
        glibc-devel) target_pkg="libc6-dev" ;;
        pkgconf-pkg-config) target_pkg="pkg-config" ;;
        git-core) target_pkg="git" ;;
        *-devel) target_pkg="lib${pkg%-devel}-dev" ;;
        *) target_pkg="$pkg" ;;
      esac
      check_cmd="dpkg -s $target_pkg"
      install_cmd="sudo apt-get install -y $target_pkg"
    else
      check_cmd="rpm -q $target_pkg"
      install_cmd="sudo dnf install -y $target_pkg"
    fi

    if ! $check_cmd &> /dev/null; then
      log "Attempting automated install of missing package: $target_pkg"
      $install_cmd
    fi
  done
  [ "$pkg_mgr" == "dnf" ] && sudo ldconfig || true
}

# prep(): Manages source acquisition and initial code patching.
# Mirrors the RPM %prep phase.
# Architectural Note: This phase handles the 'Source Remediation'. It applies
# strategic patches (like PGO optimization or autoconf compatibility) to ensure
# the upstream source is optimized for the target environment.
prep() {
...
# append_unique(): Safely merges flags into a target variable, ensuring no duplicates.
# Uses namerefs for direct modification and literal comparison to prevent
# shell expansion or PID-mangling of special characters (like $ORIGIN).
append_unique() {
  local -n var_ref=$1
  shift
  for flag in "$@"; do
...
# Architectural Note: This is the core 'Isolation Engine'. We utilize $ORIGIN
# based RPATHs to ensure the binary resolves its own libraries relative to
# its location on disk, rather than relying on system-wide paths.
compile() {
  ...
  local cflags=""
  local ldflags=""
  # ldflags_opts: Core hardening and security flags.
  # -Wl,--disable-new-dtags: Force-enables RPATH (ignoring RUNPATH) to prevent library hijacking.
  local ldflags_opts="-Wl,-Bsymbolic -Wl,-z,relro -Wl,-z,now -Wl,-s -Wl,--disable-new-dtags"
  local ssl_opts=()

  # Integrate environment flags (e.g., from RPM %optflags / %build_ldflags) at the start.
  # shellcheck disable=SC2086
  append_unique cflags ${CFLAGS:-}
  # shellcheck disable=SC2086
  append_unique ldflags ${LDFLAGS:-}

  if ${CONF[CUSTOM_LIBS]}; then
    # Configure deep-linking for isolated OpenSSL, Expat, and SQLite.
    # These prefixes reside in /opt/lib/ to maintain total isolation from /usr/lib64.
    ...
    local os_id
    os_id=$(echo "${CONF[OPENSSL_VERSION]}" | cut -d. -f1,2 | tr -d '.')

    # Extract unique flags from pkg-config.
    # shellcheck disable=SC2046
    append_unique cflags $(pkg-config --cflags expat sqlite3 openssl"${os_id}")
    # shellcheck disable=SC2046
    append_unique ldflags $(pkg-config --libs expat sqlite3 openssl"${os_id}")

    # Specialized flags to decouple from system expat.
    # We use escaped '$$ORIGIN' to ensure it is passed literally to the linker.
    local libexpat_cflags
    libexpat_cflags="$(pkg-config --cflags expat)"
    export LIBEXPAT_CFLAGS="${libexpat_cflags}"
    local libexpat_ldflags
    libexpat_ldflags="${ldflags_opts} -Wl,-rpath,'\$\$ORIGIN:\$\$ORIGIN/../lib64:\$\$ORIGIN/../..' -Wl,-rpath,${ex_root}/${CONF[PLATLIBDIR]} $(pkg-config --libs expat)"
    export LIBEXPAT_LDFLAGS="${libexpat_ldflags}"

    ssl_opts=("--with-ssl-default-suites=openssl" "--with-openssl=${os_root}" "--with-openssl-rpath=no")

    # Finalize LDFLAGS with RPATHs and hardening options.
    # We prioritize $ORIGIN to ensure local build-time binary stability.
    append_unique ldflags "-Wl,-rpath,'\$\$ORIGIN:\$\$ORIGIN/../lib64:\$\$ORIGIN/../..'"
    append_unique ldflags "-Wl,-rpath,${ex_root}/${CONF[PLATLIBDIR]}"
    append_unique ldflags "-Wl,-rpath,${sq_root}/${CONF[PLATLIBDIR]}"
    append_unique ldflags "-Wl,-rpath,${os_root}/${CONF[PLATLIBDIR]}"
    append_unique ldflags "-Wl,-rpath,${CONF[PYTHON_OPT_TARGET]}/${CONF[PLATLIBDIR]}"
    append_unique ldflags "-Wl,-z,origin"

    # Targeted Injection for 3.13+ module build logic.
    # We override the module-specific LDFLAGS to ensure isolation is non-negotiable.
    local shared_rpath="-Wl,-rpath,'\$\$ORIGIN:\$\$ORIGIN/../lib64:\$\$ORIGIN/../..'"
    export OPENSSL_LDFLAGS="${ldflags_opts} ${shared_rpath} -Wl,-rpath,${os_root}/${CONF[PLATLIBDIR]}"
    export OPENSSL_LIBS="-lssl -lcrypto"
    export SQLITE3_LDFLAGS="${ldflags_opts} ${shared_rpath} -Wl,-rpath,${sq_root}/${CONF[PLATLIBDIR]}"
    export SQLITE3_LIBS="-lsqlite3"
  else
    # Fallback to system OpenSSL headers if custom libraries are not used.
    append_unique cflags "$(pkg-config --cflags openssl || true)"
    append_unique ldflags "$(pkg-config --libs-only-L openssl || true)"
  fi

  # Apply global hardening and security options to ALL builds.
  append_unique ldflags "$ldflags_opts"
  ...
  # --with-computed-gotos: Uses direct memory jumps between bytecode instructions instead of a slower central switch statement.
  ...
  # BUILD PREP: Sanitize the build tree to prevent PGO coverage-mismatch errors.
...
# Architectural Note: This is the 'Staging Layer'. We install into a temporary
# BuildRoot to verify that the prefix-based installation (/opt/lib/python3)
# is correctly structured before final packaging.
install() {
  ...
  # We also suppress PATH warnings since we are staging into a BuildRoot.
  LD_LIBRARY_PATH=".:${CONF[INSTALL_BASE]}${CONF[PYTHON_OPT_TARGET]}/${CONF[PLATLIBDIR]}" \
    PIP_NO_WARN_SCRIPT_LOCATION=1 \
    make install DESTDIR="${CONF[INSTALL_BASE]}"

  # 2. Robustness Check & Manual Fallback (Now primary for isolation)
  # We manually trigger 'ensurepip' using the staged resident interpreter with strict
  # isolation (PYTHONHOME) to ensure it doesn't see host site-packages.
  local staged_python="${CONF[INSTALL_BASE]}${CONF[PYTHON_OPT_TARGET]}/bin/python${CONF[PBVT]}"
  local staged_home="${CONF[INSTALL_BASE]}${CONF[PYTHON_OPT_TARGET]}"
  local site_packages="${staged_home}/lib/python${CONF[PBVT]}/site-packages"

  if [ ! -d "${site_packages}/pip" ]; then
    log "Enforcing isolated pip bootstrap via ensurepip..."
    PYTHONHOME="${staged_home}" \
      LD_LIBRARY_PATH="${staged_home}/${CONF[PLATLIBDIR]}" \
      "${staged_python}" -I -m ensurepip --default-pip
  fi
  ...
  # We invoke the newly built interpreter (python3.14t) with strict isolation.
  PYTHONHOME="${CONF[INSTALL_BASE]}${CONF[PYTHON_OPT_TARGET]}" \
    LD_LIBRARY_PATH="${CONF[INSTALL_BASE]}${CONF[PYTHON_OPT_TARGET]}/${CONF[PLATLIBDIR]}" \
    ./python"${CONF[PBVT]}" -I "./pathfix${CONF[PBVT]}.py" \
...
# Architectural Note: This is the 'Environment Enrichment' phase. It ensures
# that every Python instance is shipped with a curated set of mission-critical
# PyPI packages (pip, virtualenv, etc.) linked against the isolated SSL stack.
bootstrap() {
  ...
  # Use LD_LIBRARY_PATH and PYTHONHOME to ensure total isolation from host environment.
  export LD_LIBRARY_PATH="${target_root}/${CONF[PLATLIBDIR]}"
  export PYTHONHOME="${target_root}"
  for pkg in "${pkgs[@]}"; do
    log "Enriching core bundle: $pkg"
    "$python_bin" -I -m pip install --upgrade --ignore-installed --no-warn-script-location "$pkg"
  done
  unset PYTHONHOME
  ...
  # Pathfix: Comprehensive path correction for relocatability.
  # Pip and other tools often capture the absolute BuildRoot path in shebangs,
  # .pc files, and sysconfig modules. We strip this prefix globally across the
  # installation to ensure the binaries are relocatable to /opt/lib/python3.
  log "Performing global path correction for relocatability..."
  (grep -rlI "${CONF[INSTALL_BASE]}" "${target_root}" || true) | while IFS= read -r file; do
    [ -z "$file" ] && continue
    log "Correcting leaked BuildRoot path in: ${file##*/}"
    sed -i "s|${CONF[INSTALL_BASE]}||g" "$file"
  done
  popd &> /dev/null
...
# Architectural Note: This is the 'Delta Compression' layer. For Free-Threading
# builds, we strip all artifacts already provided by the parallel GIL package
# to ensure zero-overlap and reduced artifact footprint.
cleanup_mt_delta() {
...
# Architectural Note: This is the 'Quality Gate'. If these checks fail, the
# build is blocked from packaging to prevent non-compliant binaries from
# reaching production.
validate() {
  ...
  # Uses 'readelf' to verify that the absolute build-time source path
  # has not leaked into the binary's RPATH/RUNPATH headers.
  log "Auditing RPATH headers for build-path pollution..."
  local leaks
  leaks=$(readelf -d "$python_bin" | grep RPATH | grep "${CONF[SRC_BASE]}" || true)
  if [ -n "$leaks" ]; then
    error "BUILD FAILURE: RPATH leak detected in binary! $leaks"
  fi

  # Audit 2: Deep Logic Check (Runtime level)
  # Invokes the standalone 'inspect_python.py' utility using the newly built
  # interpreter. This verifies functional integrity (SSL, Expat, etc.).
  log "Invoking deep inspection audit..."
  local custom_flag=""
  ${CONF[CUSTOM_LIBS]} && custom_flag="--custom-libs"

  export LD_LIBRARY_PATH="${target_root}/${CONF[PLATLIBDIR]}"
  export PYTHONHOME="${target_root}"
  "$python_bin" -I "${CONF[AUDITOR]}" --verbose ${custom_flag}
  unset LD_LIBRARY_PATH
  unset PYTHONHOME

  # Audit 3: Path Leaks (File Content level)
  # Scans all text files in the installation for absolute BuildRoot staging
  # paths that might break relocatability.
  log "Auditing for absolute BuildRoot path leaks in installed files..."
  local path_leaks
  path_leaks=$(grep -rI "${CONF[INSTALL_BASE]}" "${target_root}" | head -n 20 || true)
...
# Architectural Note: This is the 'Artifact Finalization' layer. It captures
# the entire prefix hierarchy into a portable, relocatable archive that
# maintains its RPATH integrity regardless of the final extraction point.
package() {
