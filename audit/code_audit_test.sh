...
# 0. Platform Detection & Path Normalization Helper
# -----------------------------------------------------------------------------
os_type=$(uname -s)
...
# We keep the RAW POSIX path for internal shell commands.
TEST_ROOT_RAW="${TMPDIR:-$TEMP}/$CONST_SUB_ROOT/$TEST_UUID"

# Export FORMATTED paths for the orchestrator consumption
MOCK_BIN=$(format_path "$TEST_ROOT_RAW/$CONST_SUB_MOCK")
export MOCK_BIN
WORKSPACE=$(format_path "$TEST_ROOT_RAW/$CONST_SUB_WS")
export WORKSPACE
MARKS=$(format_path "$TEST_ROOT_RAW/$CONST_SUB_MARKS")
export MARKS

echo "--> Initializing Test Environment: $TEST_ROOT_RAW"
mkdir -p "$TEST_ROOT_RAW/$CONST_SUB_MOCK" "$TEST_ROOT_RAW/$CONST_SUB_WS" "$TEST_ROOT_RAW/$CONST_SUB_MARKS"

# Redirect all temporary file generation and tool caches to the isolated test root
export TEMP="$TEST_ROOT_RAW"
export TMP="$TEST_ROOT_RAW"
export HOME="$TEST_ROOT_RAW/fake_home"
mkdir -p "$HOME"
export TMPDIR="$TEST_ROOT_RAW"

# Ensure cleanup on exit or interruption
trap 'echo "Cleaning up test environment..."; rm -rf "$TEST_ROOT_RAW"; rmdir "$(dirname "$TEST_ROOT_RAW")" 2>/dev/null || true' EXIT INT TERM
...
SCRIPT_PATH="$SCRIPT_DIR_RAW/code_audit.sh"
...
generate_mocks() {
  ...
  mkdir -p "$TEST_ROOT_RAW/$CONST_SUB_MOCK"
  for tool in $TOOLS; do
    ...
    bin_path="$TEST_ROOT_RAW/$CONST_SUB_MOCK/$tool"
    case "$tool" in
      "nilness")
...
touch "$TEST_ROOT_RAW/$CONST_SUB_WS/main.py" "$TEST_ROOT_RAW/$CONST_SUB_WS/requirements.txt" "$TEST_ROOT_RAW/$CONST_SUB_WS/main.go" "$TEST_ROOT_RAW/$CONST_SUB_WS/go.mod" "$TEST_ROOT_RAW/$CONST_SUB_WS/package.json" "$TEST_ROOT_RAW/$CONST_SUB_WS/index.js" "$TEST_ROOT_RAW/$CONST_SUB_WS/script.ps1" "$TEST_ROOT_RAW/$CONST_SUB_WS/module.psm1" "$TEST_ROOT_RAW/$CONST_SUB_WS/settings.psd1"

# 4. PATH Injection
# We prepend MOCK_BIN (POSIX style) to the PATH so the orchestrator calls our mocks
# instead of searching for system-installed binaries.
export OLD_PATH="$PATH"
export PATH="$TEST_ROOT_RAW/$CONST_SUB_MOCK:$PATH"
...
run_test() {
  ...
  echo "TEST: $label"
  local out_file
  # Explicitly use the test sandbox root for all temporary test logs
  out_file="$(mktemp -p "$TEST_ROOT_RAW")"
...
verify_isolation() {
  local lang_label="$1"
  shift
  (bash "$SCRIPT_PATH" "$@" || true) | grep "$lang_label" > /dev/null
}

# Test 1: Diagnostic Readiness
run_test "Tool Detection Report: Binary Check" bash "$SCRIPT_PATH" --detect
run_test "Tool Detection Report: PowerShell Assets Check" bash -c "bash $SCRIPT_PATH --detect | grep -A 3 '\[PowerShell Quality Tools\]' | grep 'pslint.ps1' | grep 'FOUND'"

# Test 2: Full Integration & Coverage Matrix
# We allow non-zero exit codes here because the orchestrator correctly reports mock findings as failures.
# The actual logic verification happens in the 'verify_language_coverage' steps below.
run_test "Full Polyglot Coverage Execution" bash -c "bash $SCRIPT_PATH --path $WORKSPACE --auto --extended --extra-scan || true"
...
run_test "Bash Toolset Coverage" verify_language_coverage Bash BASH
run_test "PowerShell Toolset Coverage" verify_language_coverage PowerShell POWERSHELL
...
rm -f "$MARKS"/ast-grep*
bash "$SCRIPT_PATH" --path "$WORKSPACE" --python --run-logic > /dev/null 2>&1 || true
...
run_test "Fix Propagation: pslint -Fix" verify_fix_flags FIX_ON pwsh -Fix
...
# Test 6: PowerShell Extended Logic
echo " --> Verifying PowerShell Extended Logic (Strict Mode)..."
rm -f "$MARKS"/*
bash "$SCRIPT_PATH" --path "$WORKSPACE" --powershell --extended > /dev/null 2>&1
run_test "PowerShell Extended: pslint -Strict executed" verify_flag_present pwsh "-Strict"
...
rm -f "$MARKS"/*
bash "$SCRIPT_PATH" --path "$WORKSPACE" --run-cleanup --golang > "$TEST_ROOT_RAW/app_warn" 2>&1
run_test "Applicability INFO: Cleanup correctly skipped for Go" grep "INFO: Phase 3 (Cleanup) is currently only applicable for Python projects." "$TEST_ROOT_RAW/app_warn"

echo " --> Verifying PowerShell Project Audit (Heuristic)..."
# Create a fresh workspace with ONLY PowerShell files
PS_WORKSPACE_RAW="$TEST_ROOT_RAW/ps_project"
mkdir -p "$PS_WORKSPACE_RAW"
touch "$PS_WORKSPACE_RAW/main.ps1" "$PS_WORKSPACE_RAW/Helper.psm1"
PS_WORKSPACE=$(format_path "$PS_WORKSPACE_RAW")
rm -f "$MARKS"/*
# Run with --auto and check if pslint was called but Python/Go were not
(bash "$SCRIPT_PATH" --path "$PS_WORKSPACE" --auto --detect > /dev/null 2>&1)
run_test "PS Project: pslint.ps1 identified via auto-detect" grep "pslint.ps1" "$MARKS/pwsh.args"
run_test "PS Project: Python tools skipped" test ! -f "$MARKS/ruff"
run_test "PS Project: Golang tools skipped" test ! -f "$MARKS/gosec"
...
RESTRICTED_PATH="$TEST_ROOT_RAW/$CONST_SUB_MOCK:/usr/bin:/bin:/usr/sbin:/sbin:/c/Windows/system32"

echo " --> Verifying Installation Trigger: Python (UV)..."
rm -f "$MOCK_BIN/vulture" # Simulate missing tool to trigger install
rm -f "$MARKS"/*
PATH="$RESTRICTED_PATH" bash "$SCRIPT_PATH" --path "$WORKSPACE" --python --install-only > "$TEST_ROOT_RAW/install_debug" 2>&1
...
echo "--> Verifying Environment Isolation (PATH Sanitization)..."
# Inject a "dirty" path containing conflicting POSIX markers
DIRTY_PATH="/usr/bin:/bin:C:/cygwin64/bin:D:/msys64/usr/bin:E:/Git/bin"
# Run --detect and capture the PATH as seen by a tool (we can use our mock marks)
rm -f "$MARKS"/*
# Test Case 1: Hardcoded Patterns
PATH="$DIRTY_PATH:$RESTRICTED_PATH" bash "$SCRIPT_PATH" --detect > /dev/null 2>&1 || true
if [ "$IS_CYGWIN" = true ]; then
  run_test "Isolation: MSYS/Git-Bash paths purged" bash -c "! grep -Ei 'msys64|Git/bin' \$MARKS/bandit.env"
else
  run_test "Isolation: Cygwin paths purged" bash -c "! grep -Ei 'cygwin64' \$MARKS/bandit.env"
fi

# Test Case 2: Dynamic *_HOME patterns
if [ "$IS_MSYS" = true ]; then
  OTHER_HOME_VAR="CYGWIN_HOME"
  OTHER_PATTERN="Custom/Cygwin"
else
  OTHER_HOME_VAR="MSYS_HOME"
  OTHER_PATTERN="Custom/MSYS"
fi
CUSTOM_DIRTY_PATH="/usr/bin:D:/$OTHER_PATTERN/bin"
rm -f "$MARKS"/*
# We set the 'other' home variable to trigger dynamic purging
env "$OTHER_HOME_VAR=D:/$OTHER_PATTERN" PATH="$CUSTOM_DIRTY_PATH:$RESTRICTED_PATH" bash "$SCRIPT_PATH" --detect > /dev/null 2>&1 || true
# Test Case 3: Backslash Normalization & Warnings
BACKSLASH_PATH="/usr/bin:C:\\cygwin64\\bin:E:\\Git\\bin"
rm -f "$MARKS"/*
# Set MSYS_HOME with backslashes to trigger potential grep warnings
MSYS_HOME="D:\\Custom\\MSYS" PATH="$BACKSLASH_PATH:$RESTRICTED_PATH" bash "$SCRIPT_PATH" --detect > "$TEST_ROOT_RAW/backslash.log" 2>&1 || true
# Verify no grep warnings were produced
run_test "Isolation: No stray backslash warnings" bash -c "! grep -q 'stray \\' \$TEST_ROOT_RAW/backslash.log"
# Verify paths were purged
if [ "$IS_CYGWIN" = true ]; then
  run_test "Isolation: Backslash paths purged" bash -c "! grep -Ei 'cygwin64|Git/bin' \$MARKS/bandit.env"
else
  run_test "Isolation: Backslash paths purged" bash -c "! grep -Ei 'cygwin64' \$MARKS/bandit.env"
fi
...
# 7. Update Lifecycle
# -----------------------------------------------------------------------------
echo "--> Verifying Parallel Status Integrity..."
# We mock a failure in 'ruff' which runs in background during Phase 1
rm -f "$MARKS"/*
# Create a failing mark for ruff
echo "1" > "$MARKS/ruff"
# Run with --parallel
if PATH="$RESTRICTED_PATH" bash "$SCRIPT_PATH" --path "$WORKSPACE" --python --parallel > /dev/null 2>&1; then
  echo "TEST: Parallel Failure Propagation"
  echo "RESULT: FAIL (Script returned 0 despite ruff failure)"
  FAILED_TESTS=$((FAILED_TESTS + 1))
else
  echo "TEST: Parallel Failure Propagation"
  echo "RESULT: PASS"
...
GLOBAL_MOCK_BIN="$TEST_ROOT_RAW/global_bin"
